...
try {
    ...
    foreach ($cart as $item) {
        ...
        if ($sku && $qty > 0) {
            try {
                $stmt = $pdo->prepare("UPDATE Artikel SET Stueckzahl = Stueckzahl - ? WHERE Artikelnummer = ? AND Stueckzahl >= ?");
                $stmt->execute([$qty, $sku, $qty]);
                if ($stmt->rowCount() === 0) {
                    // Nicht genug Bestand oder Artikel nicht gefunden
                    throw new Exception("Nicht genug Bestand für Artikel $sku");
                }
            } catch (\Throwable $updateEx) {
                error_log("Bestand-Update fehlgeschlagen für $sku: " . $updateEx->getMessage());
                throw new Exception("Fehler bei Bestandsaktualisierung");
            }
        }
    }
